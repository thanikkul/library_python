...
# Author : library Group
# Since : 2021-10-24
# Program Name : library
# Program Language : Python
# Program Purpose : library System
...
from oauth2client.service_account import ServiceAccountCredentials # จาก oauth2client.service_account เรียกใช้ module ชื่อ ServiceAccountCredentials
...
class Database:
    def __init__(self):
        self.scope  = ["https://spreadsheets.google.com/feeds", 'https://www.googleapis.com/auth/spreadsheets',  # scope คือ การกำหนดตัวแปลเพื่อเข้าถึง ati
        ...
        self.sheet1 = self.client.open('database').worksheet('Sheet1')
        self.sheet2 = self.client.open('database').worksheet('Sheet2')
        self.history = self.client.open('database').worksheet('history')
        self.cart = self.client.open('database').worksheet('cart')
...
class Function(Database):   # สร้างclass Function รับข้อมูลจาก class Node
    def __init__(self):  # สร้างฟังก์ชั่น__init__ รับข้อมูล self
        super().__init__()  # เป็นพารามิเตอร์ รับข้อมูลจาก data
    ...
    def login(self):  # สร้างฟังก์ชั่น login รับข้อมูล self
        ...
        try:  # ทำการเช็ค error
            ...
            check_Username = self.sheet1.find(Username)
            # check_Password ค้าหาคำใน sheet1 จาก password
            check_Password = self.sheet1.find(password)
    ...
    def history_insert(self, user, type, name, amount):
        ...
        self.history.append_row(data)  # เพิ่มแถวใน history รับข้อมูลจาก data
    ...
    def history_print_fromUser(self, user):
        ...
        values_list = self.history.get_all_records()
    ...
    def cart_insert(self, user, type, name, amount):
        # data รับข้อมูลมาเก็บในรูปแบบ array
        data = [user, type, name, amount, str(datetime.datetime.now().ctime())]
        self.cart.append_row(data)  # เพิ่มแถวใน cart รับข้อมูลจาก data

    def cart_print(self, user):  # สร้างฟังก์ชั่น cart_print รับค่า self,user
        # values_list รับข้อมูลมาเก็บในรูปแบบ HashMaps
        value_list = self.cart.get_all_records()
    ...
    def find_cell(self, user, name):  # สร้างฟังก์ชั่น find_call รับค่า self ,user ,name
        ...
        cell1 = self.cart.findall(user)
        # cell2 รับข้อมูลจากการค้นหาข้อมูล cart รับข้อมูลจาก name
        cell2 = self.cart.findall(name)
    ...
    def book_update(self, name, amount):
        ...
        cell_list = self.sheet2.findall(name)
    ...
    def search(self, search_term):  # สร้างฟังก์ชั่น search รับข้อมูล self และ search_term
        ...
        values_list_Book = self.sheet2.col_values(2)
        ...
        for x in match_books:  # ใช้การวนลูปค่า x ใน match_books
            ...
            match_books_Type_lower = [[j.lower() for j in i]
                                      for i in self.sheet2.get_all_values()]
            # ใช้การวนรูปค่า o,v ใน zip ของ sheet2.get_all_values() และ match_books_Type_lower
            for o, v in zip(self.sheet2.get_all_values(), match_books_Type_lower):
    ...
    def store_book(self):  # ฟังก์ชั่น store_book
        data = self.sheet2.get_all_records()  # เก็บข้อมูลไว้ในรูปแบบ array
    ...
    def check_amount_book(self, name, amount_W):  # ฟังก์ชั่น check_amount_book
        values_list = self.sheet2.col_values(2)  # รับข้อมูลมาเก็บในรูปแบบอะเรย์
        temp = 0  # กำหนดตัวแปร
        for i in values_list:  # ลูปแล้วเช็คเงื่อนไข
            temp = temp + 1
            if i == name:
                str1 = 'D'+str(temp)
                val = self.sheet2.acell(str1).value
                if amount_W > int(val):
                    return False
                else:
                    return True
...
class Display(Function):  # สร้างclass Display
    def __init__(self):  # สร้างฟังก์ชั่น__init__
        super().__init__()
...
=========================================""")  # แสดงข้อความ
        ...
        elif option == 3:  # ถ้า option == 3
            self.cart_page()  # เรียกใช้ ฟังก์ชัน cart_page
        elif option == 4:  # ถ้า option == 4
